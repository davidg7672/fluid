Clerk is already install and connected. Wire it into the Next.js app: provider, auth pages, redirects, route protection, and user menu.

## Design
User Clerk's `dark` theme from `@clerk/ui/themes` as the base.
...
Wrpa the root layout with `ClerkProvider` usign Clerk's `dark` theme.
...
- authenticated useres redirect to `/editor`
...
- all routes are protect expect public auth paths
- auth pages use CSS variables with no hardcoded colors
- `ClerkProvider` wraps teh root layout
